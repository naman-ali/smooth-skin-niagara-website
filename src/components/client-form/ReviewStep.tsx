...
import { flattenSectionQuestions, isQuestionVisible } from "@/lib/client-form/conditional";
...
function findSectionStepIndex(
  ...
  return steps.findIndex(
    (s) => s.kind === "treatment-section" && s.treatmentId === treatmentId && s.sectionId === sectionId
  );
}
    ...
    <div className="space-y-8">
      ...
      >
        <dl className="grid gap-x-6 gap-y-3 sm:grid-cols-2">
          <ReviewItem label="Name" value={`${values.clientInfo.firstName} ${values.clientInfo.lastName}`.trim()} />
          ...
          {values.clientInfo.referralSource ? (
            <ReviewItem
              label="Referral source"
              value={values.clientInfo.referralSource}
            />
          ) : null}
        </dl>
      </ReviewGroup>
      ...
      {definitions.map((definition) => {
        ...
        return (
          <ReviewGroup
            ...
            onEdit={() =>
              onEditStep(
                findSectionStepIndex(steps, definition.id, definition.sections[0].id)
              )
            }
          >
                ...
                <div key={section.id}>
                  <div className="mb-2 flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        onEditStep(findSectionStepIndex(steps, definition.id, section.id))
                      }
